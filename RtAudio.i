...
namespace std {
    %template(UIntVec) vector<unsigned int>;
    %template(IntVec) vector<int>;
}

////////
// Workarounds for nested C++ classes in SWIG (see http://www.swig.org/Doc2.0/SWIGPlus.html#SWIGPlus_nested_classes)
////////
...
%nestedworkaround RtAudio::DeviceInfo;
%nestedworkaround RtAudio::StreamParameters;
%nestedworkaround RtAudio::StreamOptions;

////////
// end Workarounds for nested C++ classes in SWIG
////////
...
%rename(set) S24::operator=;


// RtAudio version
static const std::string MY_VERSION;
...
class RtAudio
{
 public:
  ...
  ////////
  // NOTE: Nested structs moved out to global scope; see above.
  ////////
...
namespace std {
    typedef vector<int> vector<RtAudio::Api>;
}


////////
// Workarounds for nested C++ classes in SWIG (see above)
////////

%{
typedef RtAudio::DeviceInfo DeviceInfo;
typedef RtAudio::StreamParameters StreamParameters;
typedef RtAudio::StreamOptions StreamOptions;
%}
